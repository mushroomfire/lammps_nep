...
def copy_current_target(current_dir, target_subfolder):

  # This function make copy subfolder is okay in python 3.8.
  os.makedirs(target_subfolder, exist_ok=True)

  for item in os.listdir(current_dir):
    source = os.path.join(current_dir, item)
    destination = os.path.join(current_dir, target_subfolder, item)
 
    if os.path.isfile(source):
      shutil.copy2(source, destination)

    elif os.path.isdir(source) and item != target_subfolder:
      shutil.copytree(source, destination)

# copy python tree to build folder
builddir = os.path.join(olddir, 'build-python')
copy_current_target(pythondir, 'build-python')
